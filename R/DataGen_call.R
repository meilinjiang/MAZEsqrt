...
#' @noRd
DataGen_call_true.zilonm <- function(dat_placeholder, theta, K, num_Z, n, B, x1, x2, zval,
                                mval) {
    X <- rnorm(n)
    theta_trans <- trans(dat_placeholder, theta, K, xval = X, num_Z, zval = NULL)
    eps <- rnorm(n, 0, theta_trans[["delta"]])
...
#' @noRd
DataGen_call_true.zipm <- function(dat_placeholder, theta, K, num_Z, n, B, x1, x2, zval,
                              mval) {
    X <- rnorm(n)
    theta_trans <- trans(dat_placeholder, theta, K, xval = X, num_Z, zval = NULL)
    eps <- rnorm(n, 0, theta_trans[["delta"]])
